package io.teamcheeze.plum.api.core.events
...
class EventRegistry {
    companion object {
        ...
        inline fun <reified T: Event> register(noinline action: (T)->Unit): RegisteredListenerWrapper<T> {
            return RegisteredListenerWrapper(T::class.java, action).apply {
                try {
        ...
        fun <T: Event> register(clazz: Class<T>, action: (T) -> Unit): RegisteredListenerWrapper<T> {
            return RegisteredListenerWrapper(clazz, action).apply {
                try {
